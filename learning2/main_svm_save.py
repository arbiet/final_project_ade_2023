...
# Langkah 11: Menyimpan Hasil Prediksi ke dalam File JSON
# Menyimpan hasil prediksi ke dalam dictionary
results_dict = {"predictions": []}

for i, (prediction, true_label) in enumerate(zip(predictions, dummy_true_labels)):
    result_entry = {
        "citra": f"Citra {i+1}",
        "prediksi_kelas": int(prediction + 1),
        "label_sebenarnya": int(true_label)
    }
    results_dict["predictions"].append(result_entry)

# Menyimpan hasil prediksi ke dalam file JSON
output_json_filename = "hasil_prediksi.json"
with open(output_json_filename, 'w') as json_file:
    json.dump(results_dict, json_file, indent=4)

print(f"Hasil prediksi telah disimpan ke dalam file JSON: {output_json_filename}")

# Langkah 12: Menyimpan Model SVM
model_filename = os.path.join(current_directory, "model_svm.joblib")
dump(best_model, model_filename)

print(f"Model SVM telah disimpan ke dalam file: {model_filename}")
